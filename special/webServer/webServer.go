...
func listHandler(w http.ResponseWriter, r *http.Request) {
	...
	for s.Scan() {
		...
		if meta.Hash != "" {
			...
			l := fmt.Sprintf("<tr><td><a href=/download?name=%s&versions=%d>%s</a></td><td>%d</td><td>%d</td></tr>", url.PathEscape(n), meta.Version, n, meta.Version, meta.Size)
			w.Write([]byte(l))
		}
	}
	w.Write([]byte("</table>"))
...
func downloadHandler(w http.ResponseWriter, r *http.Request) {
	req, e := http.Get("http://" + os.Getenv("API_SERVER") + "/objects/" + url.PathEscape(r.URL.Query()["name"][0]) + "?versions=" + r.URL.Query()["versions"][0])
	if e != nil {
		log.Println(e)
		return
	}
